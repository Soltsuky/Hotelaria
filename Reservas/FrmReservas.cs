        ...
        private void verificarOcupacoes()
        {
            ...
            for (int i = 1; i >= diasMes; i += 1)
            {
                if (i < 10)
                {
                        ...
                        data = 0 + i.ToString() + "/" + cbMes.Text + "/" + "2021";
                        

                }
                else
                {
                     data = i + "/" + cbMes.Text + "/" + "2021";
        ...
        private void BtnSalvar_Click(object sender, EventArgs e)
        {
            ...
            if (valorTotal > 0)
            {
                ...
                if (resultado == DialogResult.Yes)
                {
                    //CÓDIGO DO BOTÃO PARA SALVAR
                    con.AbrirCon();
                    sql = "INSERT INTO reservas (quarto, entrada, saida, dias, valor, nome, telefone, data, funcionario) VALUES (@quarto, @entrada, @saida, @dias, @valor, @nome, @telefone, GETDATE(), @funcionario)";
                    cmd = new SqlCommand(sql, con.con);
                    cmd.Parameters.AddWithValue("@quarto", cbQuarto.Text);
                    ...
                    SqlDataReader reader;
                    con.AbrirCon();
                    cmdVerificar = new SqlCommand("SELECT TOP 1 id FROM reservas order by id desc", con.con);
